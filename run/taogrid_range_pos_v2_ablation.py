"""
Ablation: compare Sharpe with/without range position asymmetry v2 (top-band only).

We keep:
  - breakout risk factor ON (winner params)
  - MR+Trend OFF (per option 1 baseline)
"""
...
def main() -> None:
    out_root = Path("run/results_lean_taogrid_range_pos_v2_ablation")
    out_root.mkdir(parents=True, exist_ok=True)

    base = TaoGridLeanConfig(
        name="TaoGrid RangePos v2 Ablation",
        description="Compare Sharpe with/without range_pos asymmetry v2",
        support=111000.0,
        resistance=123000.0,
        ...
        # v2 defaults (only active if enabled)
    ...
    cfg_off = replace(base, enable_range_pos_asymmetry_v2=False, name="RangePos v2 OFF")
    cfg_on = replace(base, enable_range_pos_asymmetry_v2=True, name="RangePos v2 ON")
