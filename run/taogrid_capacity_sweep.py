"""
Sweep inventory capacity threshold for TaoGrid (market-making style).

We keep:
  - leverage = 5x
  - risk_budget_pct = 1.0 (100%)
  - fixed S/R (user-provided)
  - maker fee = 0.02% per side, slippage = 0

We sweep:
  - inventory_capacity_threshold_pct in [0.90, 0.95, 0.98, 1.00]

Outputs:
  run/results_lean_taogrid_capacity_sweep/summary.csv
  run/results_lean_taogrid_capacity_sweep/pct_*/ (metrics/trades/orders/equity + metrics_panel.*)
"""
...
def main() -> None:
    thresholds = [0.90, 0.95, 0.98, 1.00]
    ...
    out_root = Path("run/results_lean_taogrid_capacity_sweep")
    out_root.mkdir(parents=True, exist_ok=True)

    base = TaoGridLeanConfig(
        name="TaoGrid Capacity Sweep",
        description="Sweep inventory_capacity_threshold_pct under 5x leverage, maximize ROE",
        support=support,
        ...
        inventory_capacity_threshold_pct=0.90,
    )

    rows: list[dict] = []
    for pct in thresholds:
        cfg = replace(base, inventory_capacity_threshold_pct=float(pct))
        run_dir = out_root / f"pct_{pct:.2f}"
        ...
        rows.append(
            {
                "inventory_capacity_threshold_pct": pct,
    ...
    print("Capacity threshold sweep done. Top rows:")
