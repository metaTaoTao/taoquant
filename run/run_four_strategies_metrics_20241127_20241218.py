...
窗口（按日历日含尾端）：2024-11-27 ~ 2024-12-18
实现上：end 取 2024-12-19 00:00 UTC，保证覆盖到 12-18 23:59。
...
END_INCL = datetime(2024, 12, 18, tzinfo=timezone.utc)
END_EXCL = END_INCL + timedelta(days=1)  # include 12-18 full day
...
def main() -> None:
    ...
    base = TaoGridLeanConfig(
        name="4-strategy comparison (20241127-20241218)",
    ...
    r_neu = _run(cfg_neutral, data, Path("run/results_neutral_20241127_20241218"), "NEUTRAL_RANGE")
    r_bul = _run(cfg_bullish, data, Path("run/results_bullish_20241127_20241218"), "BULLISH_RANGE")
    r_bear = _run(cfg_bearish, data, Path("run/results_bearish_short_20241127_20241218"), "BEARISH_RANGE (with short overlay)")
    r_bear_lo = _run(cfg_bearish_longonly, data, Path("run/results_bearish_longonly_20241127_20241218"), "BEARISH_RANGE (long-only)")
