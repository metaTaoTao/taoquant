"""
Ablation: compare Sharpe with/without breakout risk factor.
"""
...
def main() -> None:
    out_root = Path("run/results_lean_taogrid_breakout_risk_ablation")
    out_root.mkdir(parents=True, exist_ok=True)

    base = TaoGridLeanConfig(
        name="TaoGrid Breakout Risk Ablation",
        description="Compare Sharpe with/without breakout risk factor",
        support=111000.0,
        resistance=123000.0,
        ...
        # keep MR+Trend off for clean ablation of breakout risk
        enable_mr_trend_factor=False,
    )

    cfg_off = replace(base, enable_breakout_risk_factor=False, name="Breakout OFF")
    cfg_on = replace(base, enable_breakout_risk_factor=True, name="Breakout ON")
