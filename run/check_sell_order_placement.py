...
def main():
    ...
    sell_order_placement_stats = {
        'buy_orders_executed': 0,
        'sell_orders_placed': 0,
        'sell_orders_after_buy': [],
    }
    ...
    for i, (timestamp, row) in enumerate(data.iterrows()):
        ...
        # 记录买入订单执行前的卖出订单数量
        sell_orders_before = len([o for o in runner.algorithm.grid_manager.pending_limit_orders if o.get('direction') == 'sell' and o.get('placed', False)])
        ...
        while orders_processed_this_bar < max_orders_per_bar:
            ...
            if order:
                ...
                if order['direction'] == 'buy':
                    ...
                    if executed:
                        sell_order_placement_stats['buy_orders_executed'] += 1
                        runner.algorithm.on_order_filled(order)
                        
                        # 记录买入订单执行后的卖出订单数量
                        sell_orders_after = len([o for o in runner.algorithm.grid_manager.pending_limit_orders if o.get('direction') == 'sell' and o.get('placed', False)])
                        if sell_orders_after > sell_orders_before:
                            sell_order_placement_stats['sell_orders_placed'] += 1
                            sell_order_placement_stats['sell_orders_after_buy'].append({
                                'bar': i,
                                'timestamp': timestamp,
                                'buy_level': order['level'],
                                'sell_orders_before': sell_orders_before,
                                'sell_orders_after': sell_orders_after,
                            })
                else:
                    # 卖出订单
                    executed = runner.execute_order(order, row['open'], row['close'], timestamp)
    ...
    print("卖出订单放置分析")
    print("=" * 80)
    print(f"买入订单执行次数: {sell_order_placement_stats['buy_orders_executed']}")
    print(f"卖出订单放置次数: {sell_order_placement_stats['sell_orders_placed']}")
    
    if sell_order_placement_stats['sell_orders_after_buy']:
        print(f"\n前10个买入订单执行后卖出订单放置情况:")
        for i, detail in enumerate(sell_order_placement_stats['sell_orders_after_buy'][:10]):
            print(f"  {i+1}. Bar {detail['bar']} @ {detail['timestamp']}")
            print(f"     买入订单: L{detail['buy_level']+1}")
            print(f"     卖出订单: {detail['sell_orders_before']} -> {detail['sell_orders_after']}")
    ...
    pending_sell_orders = len([o for o in runner.algorithm.grid_manager.pending_limit_orders if o.get('direction') == 'sell' and o.get('placed', False)])
    print(f"  待处理卖出订单数: {pending_sell_orders}")
