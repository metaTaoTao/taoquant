"""
Ablation: compare Sharpe with/without range position asymmetry factor.

We keep breakout risk factor ON (winner params), and compare:
  - enable_range_pos_asymmetry: False vs True
"""
...
def main() -> None:
    out_root = Path("run/results_lean_taogrid_range_pos_ablation")
    out_root.mkdir(parents=True, exist_ok=True)

    base = TaoGridLeanConfig(
        name="TaoGrid RangePos Ablation",
        description="Compare Sharpe with/without range position asymmetry",
        support=111000.0,
        resistance=123000.0,
    ...
    cfg_off = replace(base, enable_range_pos_asymmetry=False, name="RangePos OFF")
    cfg_on = replace(base, enable_range_pos_asymmetry=True, name="RangePos ON")
