"""
四个 regime 回测（同样本 bars 复用）：
- 窗口：2024-07-03 ~ 2024-07-29（UTC，含尾端）
- S/R：56k / 70k
- 数据：OKX 合约（okx_swap）本地 1m 缓存
...
END_INCL = datetime(2024, 7, 29, tzinfo=timezone.utc)
...
def main() -> None:
    dm = DataManager()
    print("=" * 80)
    print("Loading bars once (okx_swap cache)")
    print("=" * 80)
    data = dm.get_klines(
        symbol="BTCUSDT",
        timeframe="1m",
    ...
    base = TaoGridLeanConfig(
        name="4 regimes (20240703-20240729, 56k-70k)",
    ...
    tag = "20240703_20240729_sr_56k_70k"
    ...
    print("对比汇总（同样本 bars）")
    print("=" * 80)
    print(_row("NEUTRAL_RANGE", r_neu["metrics"]))
    print(_row("BULLISH_RANGE", r_bul["metrics"]))
    print(_row("BEARISH_RANGE(short)", r_bear["metrics"]))
    print("-" * 80)
    print(_row("BEARISH_LONGONLY", r_bear_lo["metrics"]))
    print("=" * 80)
